...
class HttpFileService extends FileService {
  ...
  Future<FileServiceResponse> get(String url, {Map<String, String>? headers}) async {
    final res = await dioInstance.get<ResponseBody>(
      url,
      options: Options(responseType: ResponseType.stream, headers: headers), // set responseType to `stream`
    );

    return HttpGetResponse(res);
  }
}
...
class HttpGetResponse implements FileServiceResponse {
  ...
  @override
  int? get contentLength => int.tryParse(_header('content-length') ?? '') ?? 0;
